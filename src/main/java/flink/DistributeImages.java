...
public class DistributeImages implements Callable<Void>, Serializable {
    ...
    @CommandLine.Option(names = {"--python"}, usageHelp = true, description = "Use python bridge to ctapipe to analyse data.")
    ...
    @CommandLine.Parameters(index = "2", paramLabel = "Output File for the analysis results and timestamps. Will be written as CSV.")
    String outputFile = " ";
    ...
    public Void call() throws Exception {
        ...
        if(usePython){
          System.out.println("Exectuting Python Plan");
          StreamExecutionEnvironment env = flinkPlanPython();
          env.execute();
        }
        else{
          System.out.println("Exectuting Java Plan");
    ...
    private StreamExecutionEnvironment flinkPlanPython() {
    ...
    private StreamExecutionEnvironment flinkPlan() {
        ...
        source
                ...
                .setParallelism(windowParallelism)
                .map(new MapFunction<Tuple2<ReconstrucedEvent,Double>, Tuple3<String, Double, String>>() {
                    @Override
                    public Tuple3<String, Double, String> map(Tuple2<ReconstrucedEvent, Double> value) throws Exception {
                        return new Tuple3<>(value.f0.toCSVString(), value.f1, LocalDateTime.now().toString());
                    }
                })
                .rescale()
                .writeAsCsv(outputFile, FileSystem.WriteMode.OVERWRITE)
